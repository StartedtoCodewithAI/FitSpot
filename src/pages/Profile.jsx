      ...
      <section style={{ marginTop: 40, textAlign: "left" }}>
          ...
          {messages.length === 0 ? (
            ...
            messages.map(msg => (
              <div key={msg.id} style={{ marginBottom: 8 }}>
                <b>{msg.user_name || "Anon"}</b>: {msg.content}
                <br />
        ...
        <form onSubmit={handleSendMessage} style={{ marginTop: 12, display: "flex", gap: 8 }}>
          <input
            ...
            style={{ flexGrow: 1, padding: "0.5rem 1rem", borderRadius: 12, border: "1px solid #d1d5db" }}
          />
          <FSButton type="submit" disabled={!newMsg.trim()}>
            Send
          </FSButton>
        </form>
      </section>
